...
fit_U2_score <- normalmixEM(ME_matches_filter$U2_score )
...
```{r, echo= FALSE, warning=FALSE}
write.table(ME_final[ME_P_value <= P_ME_threshold], out_filtered_ME, col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

write.table(ME_final[ME_P_value < P_ME_threshold], out_low_scored_ME , col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

write.table(ME_centric_raw[ME %in% ME_number_files_detected[N >=min_number_files_detected, ME] & len_micro_exon_seq_found<3, ], out_shorter_than_3_ME, col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
```
